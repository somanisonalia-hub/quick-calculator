        ...
        <>
          ...
          <section className={`p-6 rounded-lg border-2 ${colors.bg} ${colors.border} mb-10`}>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 gap-2">
              {categoryCalcs.map((calc) => (
                <Link key={calc.slug} href={`/${lang}/${calc.slug}`} className="text-blue-600 hover:underline text-xs sm:text-sm">
                  {calc.name}
                </Link>
              ))}
            </div>
          ...
          {otherCalcs.length > 0 && (
            <section className="mb-10">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 gap-2">
                {otherCalcs.sort((a, b) => a.name.localeCompare(b.name)).map((calc) => (
                  <Link key={calc.slug} href={`/${lang}/${calc.slug}`} className="text-blue-600 hover:underline text-xs sm:text-sm">
                    {calc.name}
                  </Link>
                ))}
